...
public class UserDetailServiceImpt implements UserDetailsService {

	private final UserInfoRepository repository;

	// アカウントロックを行うログイン失敗回数境界値
	@Value("${security.locing-border-count}")
	private int lockingBorderCount;
	// アカウントロックの継続時間
	@Value("${security.locking-time}")
	private int lokingTime;
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		var isAccountLocked = accountLockedTime != null
				&& accountLockedTime.plusHours(lokingTime).isAfter(LocalDateTime.now());
